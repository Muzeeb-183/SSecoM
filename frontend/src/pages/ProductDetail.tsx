...
import { recentlyViewedManager } from '../utils/recentlyViewed'; // ✅ Import added
...
const ProductDetail: React.FC = () => {
  ...
  // ✅ FIXED: useEffect hooks INSIDE the component
  useEffect(() => {
    if (productId) {
      fetchProduct();
    }
  ...
  // ✅ FIXED: Add product to recently viewed when product loads
  useEffect(() => {
    if (product) {
  ...
  // ✅ SIMPLIFIED: Cart context handles authentication - no duplicate checks needed
  const handleAddToCart = async () => {
    ...
    try {
      ...
      // Only show success message if user is authenticated (cart context handles redirect)
      if (isAuthenticated) {
        toast.success(
          <div className="flex items-center space-x-2">
            <span className="text-lg">🔥</span>
            <div>
              <div className="font-bold">Added to Fire Cart!</div>
  ...
  // ✅ SIMPLIFIED: Quick add to cart for related products
  const handleQuickAddToCart = async (relatedProduct: Product, event: React.MouseEvent) => {
  ...
  return (
    <>
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-6">
              ...
              <div className="relative bg-white rounded-3xl shadow-2xl shadow-orange-500/20 overflow-hidden border-4 border-orange-500/30 p-8">
                {product.images[selectedImageIndex] ? (
                  <img
                    src={getImageForContext(product.images[selectedImageIndex], 'detail')}
                    alt={product.name}
              ...
              {product.images.length > 1 && (
                <div className="flex space-x-3 overflow-x-auto pb-2 px-2">
                  {product.images.map((image, index) => (
                    ...
                    >
                      <img
                        src={getImageForContext(image, 'thumbnail')}
            ...
            <div className="space-y-8">
              ...
              <div>
                ...
                {/* ✅ ENHANCED PRICING SECTION WITH SIMPLIFIED CART INTEGRATION */}
                <div className="mb-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {/* ✅ SIMPLIFIED ADD TO CART BUTTON - Context handles auth */}
                    <button
                      onClick={handleAddToCart}
                      disabled={isAddingToCart || cartLoading}
                      className={`
                        px-6 py-3 rounded-xl font-bold text-white border-2 
                        transition-all duration-300 transform hover:scale-105 
                        shadow-lg hover:shadow-xl
                        ${(isAddingToCart || cartLoading)
                          ? 'bg-gray-600 border-gray-500 cursor-not-allowed' 
                          : 'bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 border-blue-400 hover:border-purple-400'
                        }
                      `}
                    ...
                    >
                      {(isAddingToCart || cartLoading) ? (
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <span>{!isAuthenticated ? 'Login to Add to Cart' : 'Add to Cart'}</span>
                ...
                {relatedProducts.map((relatedProduct) => (
                  ...
                  >
                    ...
                    <div className="p-6">
                      ...
                      {/* ✅ SIMPLIFIED ACTION BUTTONS - Context handles auth */}
                      <div className="flex space-x-2">
                        <button
                          ...
                          className="flex-1 bg-gradient-to-r from-orange-600 to-red-600 text-white py-3 px-4 rounded-xl font-semibold text-sm hover:from-orange-700 hover:to-red-700 transition-all transform hover:scale-105 border border-orange-400"
                        >
                          View Product
                        </button>
                        
                        {/* ✅ Simplified Quick Add to Cart Button */}
                        <button
                          onClick={(e) => handleQuickAddToCart(relatedProduct, e)}
                          className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-3 rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all transform hover:scale-105 border border-blue-400"
                          title={!isAuthenticated ? 'Login required to add to cart' : 'Add to cart'}
                        >
      ...
      {/* ✅ Cart Sidebar - Properly placed at the end */}
      <CartSidebar 
